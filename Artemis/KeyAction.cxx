	...
	short key_action_manager::register_action(key _Key, std::function<void()>&& _Action) noexcept {
		for (short i = 0; i < this->_IdMap.size(); i++)
			if (!this->_IdMap.test(i)) {
				this->_IdMap.set(i);
				this->_DirectKeyActions.push_back({ _::__execution_context::get(), std::make_pair(_Key, std::move(_Action)), i });
				return i;
			}
		return -1;
	}
	...
	void key_action_manager::invoke(UINT _WindowMessage, key _Key) const noexcept {
		...
		case WM_KEYDOWN:
			for (const auto& o : this->_KeyDownActions)
				if (o._Object->contained_key() == _Key) {
					_::__execution_context::set(o._LoadedObjectOwner);
					net.exec_l2([&o]() { o._Object->on_key_down(); });
				}
			...
			for (const auto& o : this->_DirectKeyActions)
				if (o._Object.first == _Key) {
					_::__execution_context::set(o._LoadedObjectOwner);
					net.exec_l2(o._Object.second);
				}
			break;
		...
		case WM_KEYUP:
			for (const auto& o : this->_KeyUpActions)
				if (o._Object->contained_key() == _Key) {
					_::__execution_context::set(o._LoadedObjectOwner);
					net.exec_l2([&o]() { o._Object->on_key_up(); });
				}
			for (const auto& o : this->_KeyActions)
				if (o._Object->contained_key() == _Key) {
					_::__execution_context::set(o._LoadedObjectOwner);
